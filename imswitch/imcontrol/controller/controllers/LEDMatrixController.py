...
class LEDMatrixController(ImConWidgetController):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._setupInfo.LEDMatrix is None:
            return

        self.camera = self._setupInfo.LEDMatrix.camera
        self.positioner = self._setupInfo.LEDMatrix.positioner
        #self._master.detectorsManager[self.camera].crop(*self.cropFrame)

        # Connect LEDMatrixWidget buttons
    ...
    def focusButton(self):
        ...
        self.LEDMatrix(rangez,resolutionz)
        self._widget.focusButton.setText('LEDMatrix')

    @APIExport(runOnUIThread=True)
    # Update focus lock
    def LEDMatrix(self, rangez=100, resolutionz=10):
