...
class MCTController(ImConWidgetController):
    ...
    def stopMCT(self):
        ...
        try:
            self.stages.move(value=(self.initialPosition[0], self.initialPosition[1]), axis="XY", is_absolute=True,
                             is_blocking=True)
        except:
            pass
    ...
    def showLast(self):
        ...
        try:
            ...
            if isCleanStack:
                LastStackLaser1ArrayLast = self.cleanStack(self.LastStackLaser1ArrayLast)
            else:
                LastStackLaser1ArrayLast = self.LastStackLaser1ArrayLast
        ...
        try:
            if isCleanStack:
                LastStackLaser2ArrayLast = self.cleanStack(self.LastStackLaser2ArrayLast)
            else:
                LastStackLaser2ArrayLast = self.LastStackLaser2ArrayLast
    ...
    def takeImageIllu(self, illuMode, intensity, timestamp=0):
        ...
        tiledImageDimensions = (
        nTilesX * imageDimensions[0] // downScaleFactor, nTilesY * imageDimensions[1] // downScaleFactor)
        self.tiledImage = np.zeros(tiledImageDimensions)

        # initialize xy coordinates
        self.stages.move(value=(self.xScanMin + self.initialPosition[0], self.yScanMin + self.initialPosition[1]),
                         axis="XY", is_absolute=True, is_blocking=True)
        ...
        for ipos, iXYPos in enumerate(xyScanStepsAbsolute):
            ...
            self.stages.move(value=(iXYPos[0] + self.initialPosition[0], iXYPos[1] + self.initialPosition[1]),
                             axis="XY", is_absolute=True, is_blocking=True)
            ...
            lastFrameScaled = cv2.resize(lastFrame, None, fx=1 / downScaleFactor, fy=1 / downScaleFactor,
                                         interpolation=cv2.INTER_NEAREST)
            self.tiledImage[int(iX * imageDimensionsDownscaled[0]):int(
                iX * imageDimensionsDownscaled[0] + imageDimensionsDownscaled[0]),
            int(iY * imageDimensionsDownscaled[1]):int(
                iY * imageDimensionsDownscaled[1] + imageDimensionsDownscaled[1])] = lastFrameScaled
