...
class MultiFociCalibTarget(TargetBase):
    ...
    def __init__(self, **params):
        ...
        if self.section_size is None:
            raise ValueError (
                f"Target {self.target_type} needs section_size, but found section_size = None"
                )
        if not isinstance(self.section_size,tuple):
            raise TypeError(
                f"Section size is expected to be a tuple with (height, width)"
            )
    ...
    def _set_param(self, key, value):
        ...
        elif key == "period_x_nm":
            self.period_x = value

        elif key == "period_y_nm":
            self.period_y = value
