...
class SIMManager(SignalInterface):
    ...
    def __init__(self, simInfo, *args, **kwargs):
        ...
        self.__correctionPatternsDir = self.__simInfo.correctionPatternsDir
        self.__maskLeft = Mask(self.__simSize[1], int(self.__simSize[0] / 2), self.__wavelength)
        self.__maskRight = Mask(self.__simSize[1], int(self.__simSize[0] / 2), self.__wavelength)
    ...
    def initCorrectionMask(self):
        ...
        self.__maskCorrection = Mask(self.__simSize[1], int(self.__simSize[0]), self.__wavelength)
    ...
    def initTiltMask(self):
        ...
        self.__maskTiltLeft = Mask(self.__simSize[1], int(self.__simSize[0] / 2),
                                   self.__wavelength)
        self.__maskTiltLeft.setTilt(self.__angleMount, self.__pixelsize)
        self.__maskTiltRight = Mask(self.__simSize[1], int(self.__simSize[0] / 2),
                                    self.__wavelength)
        self.__maskTiltRight.setTilt(-self.__angleMount, self.__pixelsize)

    def initAberrationMask(self):
        # Add blazed grating tilting mask
        self.__maskAberLeft = Mask(self.__simSize[1], int(self.__simSize[0] / 2),
                                   self.__wavelength)
        self.__maskAberLeft.setBlack()
        self.__maskAberRight = Mask(self.__simSize[1], int(self.__simSize[0] / 2),
                                    self.__wavelength)
...
class Mask:
    """Class creating a mask to be displayed by the SIM."""
