...
class ThorlabsShutterManager(ShutterManager):
    """ ShuterManager for analog-value NI-DAQ-controlled shutters.
    Manager properties:
    - "serial_no_shutter": serial number of the Thorlabs shutter
    """

    def __init__(self, shutterInfo, name, **lowLevelManagers):
        self.__logger = initLogger(self, tryInheritParent=True)
        super().__init__(shutterInfo, name)  # Appel du constructeur de la classe de base
        
        # Initialize Thorlabs shutter
        self._shutter_serial_no = shutterInfo.managerProperties['serial_no_shutter']
